...
export const TabButton: React.FC<TabButtonProps> = ({ icon, active, onClick, special }) => {
  ...
  return (
    <button 
      onClick={onClick}
      className={`
        flex flex-col items-center justify-center w-[20%] h-full transition-all duration-300 ease-out
        ${active ? 'text-black dark:text-white scale-110 opacity-100' : 'text-black opacity-40 hover:opacity-60 dark:text-white dark:opacity-40 hover:scale-105'}
      `}
    >
